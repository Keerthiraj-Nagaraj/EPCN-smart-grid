...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 27 01:14:54 2020
...
iinj = pd.read_csv('../data csv files/coord/IAT_inject_multi_FDI_0_97chai_point1percent_noise_7_22_new.csv')#, header = None) # IAT Injection
tdinj = pd.read_csv('../data csv files/coord/TD_inject_multi_FDI_0_97chai_point1percent_noise_7_22_new.csv')#, header = None) # TD Injection
orz = pd.read_csv('../data csv files/coord/orz_mfdi_coord_97_722.csv')

err_ins = pd.read_csv('../data csv files/coord/err_ins_mfdi_coord_97_722.csv')
z = pd.read_csv('../data csv files/coord/z_mfdi_coord_97_722.csv')
...
# etas = [5.25, 5.5, 5.75, 6.0, 6.25, 6.50, 6.75, 7.25, 7.5, 7.75, 8.0, 8.25, 8.5, 8.75]
# etas = [3, 5, 5.25, 5.5, 5.75, 6.0, 6.25, 6.50, 6.75, 7, 7.25, 7.5, 7.75, 8.0, 8.25, 8.5, 8.75, 9, 11]

# etas = [3,5,6,7,8,9,11,13,15,17,19,21]

etas = [11]
betas = [90] #[5, 15, 45, 90, 150, 225, 450, 675, 900, 1350]
alphas = [0.00008]

cm_title = "CECD-AS coord multiFDI 97 722 new CECD cv" 
...
for j in range(1,119):
     ...
     for i in range(len(ed)):
          if ed[i][0] == j or ed[i][1] == j:
               ...
               feat_ind.append(len(ed) + ed[i][2])
               feat_ind.append(2 * len(ed) + ed[i][2])
...
iinj = iinj.drop(iinj.columns[0], axis=1) # remove column 0 (the feature numbers)
iinj_arr = np.array(iinj)
# iinj_arr = iinj_arr.reshape(21600,691)

tdinj = tdinj.drop(tdinj.columns[0], axis=1) # remove column 0 (the feature numbers)
tdinj_arr = np.array(tdinj)
# tdinj_arr = tdinj_arr.reshape(21600,691)
...
data_iinj = np.transpose(iinj_arr)
cov_ind = np.var(data_iinj, axis = 0)
zero_variance_features = np.where(cov_ind == 0)
zero_variance_features = list(zero_variance_features[0])

df_data_iinj = pd.DataFrame(data_iinj)
df_data_iinj.drop(df_data_iinj.columns[zero_variance_features],axis=1,inplace=True)
data_iinj = np.array(df_data_iinj)

#Normalize
for i in range(data_iinj.shape[1]):
     data_iinj[:,i] = (data_iinj[:,i] - np.mean(data_iinj[:,i]))/(np.std(data_iinj[:,i]))

# In[]
################### TD Injection
     
data_tdinj = np.transpose(tdinj_arr)

cov_ind = np.var(data_tdinj, axis = 0)

zero_variance_features = np.where(cov_ind == 0)
zero_variance_features = list(zero_variance_features[0])

df_data_tdinj = pd.DataFrame(data_tdinj)
df_data_tdinj.drop(df_data_tdinj.columns[zero_variance_features],axis=1,inplace=True)
data_tdinj = np.array(df_data_tdinj)

#Normalize

for i in range(data_tdinj.shape[1]):
     data_tdinj[:,i] = (data_tdinj[:,i] - np.mean(data_tdinj[:,i]))/(np.std(data_tdinj[:,i]))
...
# =============================================================================
data = np.concatenate((data_sg, data_iinj, data_tdinj), axis = 0)
...
stop_data_index = 21600 #21600 #12600
...
result_table = np.zeros((num_of_trials + 2, 15))
...
# =============================================================================
                  ...
                  curr_sim_time_2 = time.time()
                  
                  curr_sim_dur = curr_sim_time_2 - curr_sim_time_1
                  
                  curr_result = [eta_val, beta_val, alpha_val, crossvalidation_number, tn, fp, 
                                 fn, tp, acc, prec, recall, f1, curr_sim_time_1, curr_sim_time_2, curr_sim_dur]
                 
                  print('eta, beta, alpha, tn, fp, fn, tp, acc, prec, recall, f1, t1, t2, dt: ', curr_result)
                 
                  result_table[trial_num,:] = [eta_val, beta_val, alpha_val, crossvalidation_number, tn, fp, 
                                               fn, tp, acc, prec, recall, f1, curr_sim_time_1, curr_sim_time_2, curr_sim_dur]
...
# result_table_pd.to_csv(file_title + 'result_table_coord_final_betas.csv')
...
# result_table_pd = pd.DataFrame(result_table)
# result_table_pd.to_csv('local csv files eta 11/result_table_coord_cecd_CV.csv')

#%%
result_table_pd = pd.DataFrame(rx_matrix)
result_table_pd.to_csv('local csv files eta 11/cecd final/cecd_rx_mat_mfdi.csv')

result_table_pd = pd.DataFrame(thr_matrix)
result_table_pd.to_csv('local csv files eta 11/cecd final/cecd_thr_mat_mfdi.csv')

result_table_pd = pd.DataFrame(y_pred)
result_table_pd.to_csv('local csv files eta 11/cecd final/cecd_y_pred_mfdi.csv')

result_table_pd = pd.DataFrame(ecd_decision_scores)
result_table_pd.to_csv('local csv files eta 11/cecd final/cecd_score_mfdi_cecd_cv.csv')
