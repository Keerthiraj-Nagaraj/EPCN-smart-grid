...
data_pd_iat = pd.read_csv('../data csv files/IAT_inject_multi_FDI_0_97chai_Version2.csv')
data_pd_iat = data_pd_iat.drop(data_pd_iat.columns[0], axis = 1)
data_iat = np.array(data_pd_iat)

#%%

data_pd_td = pd.read_csv('../data csv files/TD_inject_multi_FDI_0_97chai_Version2.csv')
...
for i in range(len(data_iat)):
    fp_iat =  open("local csv files/data_source_2_fdi_iat.csv", "a", newline="")
    writer = csv.writer(fp_iat, delimiter=",")
        # writer.writerow(["your", "header", "foo"])  # write header
    writer.writerow(data_iat[i,:])
    fp_iat.close()
    
    fp_td =  open("local csv files/data_source_2_fdi_td.csv", "a", newline="")
