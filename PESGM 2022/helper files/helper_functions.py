...
def train_model(train_data, 
    ...
    rx_matrix_train = np.zeros((len(train_data), len(number_of_regions)))
    ...
    rxdist_normal = rx_matrix_train[idx_normal_train, :]


    return mu_list, icov_list, rxdist_normal

#%%


def compute_threshold(rxdist_normal, beta, eta, number_of_regions):
    
    thr_update_list_len = beta
    eta_val = eta
    
    thr_list = []
    
    for i in number_of_regions:
        rxdist = list(rxdist_normal[:,i])
        curr_thr = np.mean((rxdist[-thr_update_list_len:]) + eta_val * np.std(rxdist[-thr_update_list_len:]))
        thr_list.append(curr_thr)
...
def sample_inference(sample_data_point, mu_list, icov_list, feat, thr_list, number_of_regions):
    
    anomaly_vector = np.zeros((1, number_of_regions))
    
    for i in range(number_of_regions):
      
        curr_sample = sample_data_point[:, feat[i]]
        
        rxdist = RXdistance(curr_sample, mu_list[i], icov_list[i]) 
        
        if curr_sample.shape[0] > 1:
            anomaly_vector[0,i] = int(rxdist[0][0] > thr_list[i])
        else:
            anomaly_vector[0,i] = int(rxdist[0] > thr_list[i])
    
    
    overall_anomaly_decision = np.sum(anomaly_vector, axis = 1)
    
    anomaly_regions = np.where(anomaly_vector > 0)[0]
    
    return overall_anomaly_decision, anomaly_regions

#%%
    
def update_model(sample_data_point,
                 mu_list,
                 icov_list,
                 alpha,
                 feat,
    ...
    for i in range(number_of_regions):
        ...
        curr_mu = mu_list[i]
        curr_icov = icov_list[i]
        
        if alpha != 0:
            
             mu = (1-alpha) * curr_mu + alpha * curr_sample           
             icov = (1/(1-alpha)) * (curr_icov - ( (curr_sample - mu) * np.transpose((curr_sample*mu)) / ( ((1-alpha)/alpha) + np.transpose((curr_sample*mu)) * (curr_sample - mu))))
        
             curr_mu = mu
             curr_icov = icov

        mu_list[i] = curr_mu
        icov_list[i] = curr_icov
        
    return mu_list, icov_list
