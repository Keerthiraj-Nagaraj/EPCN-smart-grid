...
# sparam_simulfdi_se_norm.py --> sample wise parameter + simul FDI attack detection with ECD-AS
...
print('simul/sparam_simulfdi_cv')
print('')
...
suffix_name = '_eta_14_12_alpha_1e_5'
...
orz_pd = pd.read_csv('../data csv files latest/att_01-08-2018_sparam_simul.csv')
...
se_residual_pd = pd.read_csv('../data csv files latest/sparam_simul_residual.csv')
# se_estimate_pd = pd.read_csv('../data csv files latest/estimate_sparam_simulfdi.csv')

se_score = pd.read_csv('../data csv files latest/sparam_simul_se_score.csv')
se_threshold = pd.read_csv('../data csv files latest/sparam_simul_se_threshold.csv')

#%%

print('Ground truth - parameter attacks')

line_buses_sparam_pd = pd.read_csv('../data csv files latest/line_buses_sparam_simul.csv')
...
err_in_pd = pd.read_csv('../data csv files latest/err_ins_sparam_simul.csv')
...
result_table_pd.to_csv('sparam_simul_result_table_ml_fdi_cv_14.csv')


se_result_table_pd = pd.DataFrame(se_result_table)
se_result_table_pd.to_csv('sparam_simul_result_table_se_cv.csv')
...
y_test_overall = np.zeros((2000, num_of_trials))
# se_decision_scores = np.zeros((stop_data_index - train_size, num_of_trials))
          ...
          for beta_val in betas:
               ...
               for alpha_val in alphas:
                    ...
                    # =============================================================================
                         ...
                         y_pred_overall_sparam[:,trial_num] = y_pred
...
sparam_result_table_pd.to_csv('sparam_simul_result_table_ml_sparam_cv_15.csv')

#%%

# =============================================================================
# Result table for both attacks
# =============================================================================

ytest_both = y_test_overall
ypred_both = np.zeros(ytest_both.shape)

ypred_both[y_pred_overall_fdi>0] = 1
ypred_both[y_pred_overall_sparam>0] = 1


#%%


simul_both_result_table = np.zeros((num_of_trials + 2, 12))
...
for cv_curr_num in range(cv_num):
    ...
    # curr_result = [eta_val, beta_val, alpha_val, crossvalidation_number, tn, fp, fn, tp, acc, prec, recall, f1]
    # print('ML - ', curr_result)
    simul_both_result_table[trial_num,:] = [eta_val, beta_val, alpha_val, crossvalidation_number, tn, fp, fn, tp, acc, prec, recall, f1]
    
    trial_num += 1    


simul_both_result_table[trial_num,:] = np.mean(simul_both_result_table[:num_of_trials,:], axis = 0)
trial_num += 1

simul_both_result_table[trial_num,:] = np.std(simul_both_result_table[:num_of_trials,:], axis = 0)

#%%

simul_both_result_table_pd = pd.DataFrame(simul_both_result_table)
simul_both_result_table_pd.to_csv('sparam_simul_result_table_ml_both_cv.csv')
