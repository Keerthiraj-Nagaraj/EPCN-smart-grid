...
orz = pd.read_csv('orz_drift.csv')
err_ins = pd.read_csv('err_ins_drift.csv')
err_id = pd.read_csv("err_id_drift.csv", header = None)
se_dist = pd.read_csv("se_dist_drift.csv")
z = pd.read_csv('z_drift.csv')
...
for j in range(1):
     ...
     for i in range(10):
          ...
          if i<3:
               x_train.extend(data[data_splits[i][0]:data_splits[i][1], :])
               y_train.extend(y_data[data_splits[i][0]:data_splits[i][1]])
               se_pred_train.extend(y_se[data_splits[i][0]:data_splits[i][1]])
          else:
               x_test.extend(data[data_splits[i][0]:data_splits[i][1], :])
               y_test.extend(y_data[data_splits[i][0]:data_splits[i][1]])
               se_test.extend(se_distance[data_splits[i][0]:data_splits[i][1]])
               se_pred_test.extend(y_se[data_splits[i][0]:data_splits[i][1]])
...
for i in range(len(feat)):
     ...
     curr_thr = np.mean(curr_rxdist) + 7.5 * np.std(curr_rxdist)
...
plot_confusion_matrix(cm, target_names, title='Confusion_Matrix_ECDT')
...
#se_predictions = pd.read_csv('se_pred_best_test_drift.csv')
...
target_names = ['Normal', 'Anomalies']
print(classification_report(y_test, se_pred, target_names=target_names))
plot_confusion_matrix(cm_SE, target_names, title='Confusion_Matrix_SE_ECDT_drift')
...
plot_confusion_matrix(cm_and, target_names, title='Confusion_Matrix_ECDT_drift_AND')
...
plot_confusion_matrix(cm_or, target_names, title='Confusion_Matrix_ECDT_drift_OR')
...
#case118_drift_10000_20190515T161311.mat

anomaly_matrix_df = pd.DataFrame(anomaly_matrix)
anomaly_matrix_df.to_csv('anomaly_matrix_08_21_drift_ECDT.csv')


rx_matrix_df = pd.DataFrame(rx_matrix)
rx_matrix_df.to_csv('rx_matrix_08_21_drift_ECDT.csv')


rx_full_df = pd.DataFrame(rx_full)
rx_full_df.to_csv('rx_full_08_21_drift_ECDT.csv')


thr_list_df = pd.DataFrame(thr_list)
thr_list_df.to_csv('thr_list_08_21_drift_ECDT.csv')



an_df = pd.DataFrame(an)
an_df.to_csv('anomaly_predictions_region_wise_08_21_drift_ECDT.csv')
...
for i in range(len(y_test)):
     if y_pred[i] == 0:
          ecdt_drift.append(np.min(rx_matrix[i,:]))
     else:
          ecdt_drift.append(np.max(rx_matrix[i,:]))

ecdt_drift = np.array(ecdt_drift)

      
ecdt_drift_pd = pd.DataFrame(ecdt_drift)
ecdt_drift_pd.to_csv('ecdt_drift.csv')


y_test_pd = pd.DataFrame(y_test)
y_test_pd.to_csv('y_test_drift.csv')

y_pred_pd = pd.DataFrame(y_pred)
y_pred_pd.to_csv('y_pred_ecd_sg.csv')
...
#fn_sample_number_df.to_csv('FN_sample_number_08_21_drift_cordet.csv')
#
#fn_regions_df = pd.DataFrame(fn_regions)
#fn_regions_df.to_csv('FN_regions_08_21_drift_cordet.csv')
#
#fp_sample_number_df = pd.DataFrame(fp_sample_numbers)
#fp_sample_number_df.to_csv('FP_sample_number_08_21_drift_cordet.csv')
#
#import csv
#
